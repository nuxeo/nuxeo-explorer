...
public class TestSnapshotListener extends AbstractApidocTest {
    ...
    protected void check(Integer expected, Long actual) {
        if (expected == null) {
            assertNull(actual);
        } else {
    ...
    public void testExtensionPointOrders() {
        ...
        check(18, listener.getExtensionPointTotal(
                ExtensionPointInfo.computeId("org.nuxeo.runtime.ConfigurationService", "configuration")));
        check(0, listener.getExtensionRegistrationOrder(ExtensionInfo.computeId(SNAPSHOT_COMP, "configuration", 0)));
        check(1, listener.getExtensionRegistrationOrder(ExtensionInfo.computeId(SNAPSHOT_COMP, "configuration", 1)));
        // self registration
