/*
 * (C) Copyright 2020-2023 Nuxeo (http://nuxeo.com/) and others.
 *
...
pipeline {
  ...
  environment {
    ...
    JIRA_NUXEO_ADDON_MOVING_VERSION = 'explorer-2023.x'
  ...
  stages {
      ...
      steps {
        container('maven') {
          script {
            ...
            nxMvn.download(artifact: "org.nuxeo.packages:nuxeo-platform-explorer-package:${BUILD_VERSION}:zip")
            nxUtils.postForm(credentialsId: 'connect-prod', url: "${CONNECT_PROD_SITE_URL}marketplace/upload?batch=true",
                form: ["package=@target/nuxeo-platform-explorer-package-${BUILD_VERSION}.zip"])
          }
        }
      }
    }
    ...
    stage('Release Jira version') {
      steps {
        container('maven') {
          script {
            def jiraVersionName = "explorer-${VERSION}"
            // create a new released version in Jira
            def jiraVersion = [
                project: 'NXP',
                name: jiraVersionName,
                description: "Explorer Addon ${VERSION}",
